...
engine_dataset = create_engine(
    f'postgresql://{USER}:{PASSWORD}@localhost:{PORT}/{DB2}')
engine.execute(
...
with column_1:
    ...
    if st.button('Save record to database'):
        ...
        st.info('Name: **%s** and details: **%s** saved to database' %
                (name, details[1:-1]))
    ...
    if field == 'name':
        ...
        if st.button('Update records'):
            ...
            st.info('Updated name to **%s** in record **%s**' %
                    (updated_name, name_key))
    elif field == 'details':
        updated_details = st.text_input(
            'Please enter updated details (separated by comma)')
        updated_details = ('{%s}' % (updated_details))
        if st.button('Update records'):
            update_record(field, name_key, updated_details, engine)
            st.info('Updated details to  **%s** in record **%s**' %
                    (updated_details[1:-1], name_key))
    ...
    if st.button('Search'):
        ...
        st.info('Record name is **%s**, record details is **%s**' %
                (read_name, str(read_details)[1:-1]))
...
with column_2:
    ...
    try:
        ...
        dataset_to_read = st.selectbox('Please select dataset to read', ([
                                       x[0] for x in list_datasets(engine_dataset)]))
